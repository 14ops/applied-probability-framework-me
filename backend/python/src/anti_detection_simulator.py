...
class HumanBehaviorEmulator:
    def __init__(self, min_delay_ms=50, max_delay_ms=200, long_pause_interval=10, long_pause_min_s=5, long_pause_max_s=30):
        self.min_delay_ms = min_delay_ms
        self.max_delay_ms = max_delay_ms
        self.long_pause_interval = long_pause_interval # Every N actions, a long pause might occur
    ...
    def apply_human_jitter(self, action_name):
        ...
        if self.action_count % self.long_pause_interval == 0 and random.random() < 0.3: # 30% chance for a long pause
            long_pause_duration = random.uniform(self.long_pause_min_s, self.long_pause_max_s)
...
class DetectionSimulator:
    def __init__(self, aggressiveness=0.5):
        ...
        self.mouse_movements = deque(maxlen=5) # Keep track of last 5 mouse movements (conceptual)
    ...
    def _evaluate_mouse_path(self, mouse_path):
        ...
        if len(mouse_path) > 2:
            ...
            for i in range(1, len(mouse_path) - 1):
                ...
                if abs((end[1] - start[1]) * p[0] - (end[0] - start[0]) * p[1] + end[0] * start[1] - end[1] * start[0]) < 0.1:
    ...
    def update_detection_score(self, action_type, action_details=None):
        ...
        if action_type == "mouse_click" and action_details and "mouse_path" in action_details:
        ...
        self.detection_score = max(0, self.detection_score * 0.95 + score_increase) # Decay over time
...
if __name__ == "__main__":
    emulator = HumanBehaviorEmulator(min_delay_ms=100, max_delay_ms=500, long_pause_interval=5, long_pause_min_s=2, long_pause_max_s=10)
