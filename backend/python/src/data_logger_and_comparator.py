...
class BasicStrategy:
    def apply(self, simulator):
        ...
        if unrevealed_cells:
            ...
            if simulator.cells_clicked > 1 and random.random() < 0.3: # 30% chance to cash out after 1+ clicks
                simulator.cash_out()
        else:
...
class DataComparator:
    ...
    def compare_distributions(self, real_data, simulated_data, metrics=None):
        ...
        if "win_rate" in metrics:
            ...
            print(f"Win Rate - Real: {real_win_rate:.2%}, Simulated: {sim_win_rate:.2%}, Diff: {win_rate_diff_val:.2%}")
        ...
        if "payout_distribution" in metrics:
            ...
            sim_payouts = [s["final_payout"] for s in simulated_data if "final_payout" in s]
            if real_payouts and sim_payouts:
                ...
                print(f"Avg Payout - Real: {real_avg_payout:.2f}, Simulated: {sim_avg_payout:.2f}, Diff: {avg_payout_diff_val:.2f}")
...
def run_data_collection_and_comparison(num_real_sessions=10, num_simulated_sessions=100):
    logger = DataLogger()
    ...
    for i in range(num_real_sessions):
        ...
        while not sim.game_over:
            ...
            time.sleep(random.uniform(0.01, 0.05)) # Reduced delay for faster simulation
            strategy.apply(sim)
    ...
    comparison_results = comparator.compare_distributions(real_game_sessions, simulated_sessions)
    ...
    print("For example, if simulated win rate is consistently off, adjust game_simulator parameters or strategy logic.")
