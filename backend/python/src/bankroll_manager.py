...
class MoneyManager:
    def __init__(self, bankroll_simulator, stop_loss_percent=0.5, drawdown_reduction_threshold=0.2, drawdown_reduction_factor=0.5):
        ...
        self.drawdown_reduction_factor = drawdown_reduction_factor # e.g., 0.5 means halve the stake
        self.original_bet_size = bankroll_simulator.initial_bet_size
        self.current_bet_size = bankroll_simulator.initial_bet_size

    def check_stop_loss(self):
        if self.bankroll_simulator.get_current_bankroll() <= self.bankroll_simulator.initial_bankroll * (1 - self.stop_loss_percent):
            print(f"!!! STOP LOSS TRIGGERED! Bankroll at {self.bankroll_simulator.get_current_bankroll():.2f}")
            return True
        return False
    ...
    def adjust_bet_size(self):
        ...
        if current_drawdown >= self.drawdown_reduction_threshold:
            ...
            new_bet_size = max(self.original_bet_size * 0.1, self.current_bet_size * (1 - self.drawdown_reduction_factor))
            ...
            if current_drawdown < self.drawdown_reduction_threshold / 2 and self.current_bet_size < self.original_bet_size:
    ...
    def get_recommended_bet_size(self, p_win=None, net_odds=None, kelly_fraction=0.5):
        ...
        if p_win is not None and net_odds is not None:
            ...
            bet_from_kelly = kelly_bet_fraction * self.bankroll_simulator.get_current_bankroll()
        
        # Combine Kelly with dynamic stake sizing, ensuring a minimum bet
        # Use max(1.0, ...) to ensure a minimum bet of 1 unit
        return max(1.0, min(bet_from_kelly if bet_from_kelly > 0 else self.current_bet_size, self.current_bet_size))
...
if __name__ == "__main__":
    ...
    mm = MoneyManager(sim, stop_loss_percent=0.2, drawdown_reduction_threshold=0.15, drawdown_reduction_factor=0.5)
    ...
    for i in range(num_games):
        if sim.is_ruined() or mm.check_stop_loss():
            print(f"Simulation stopped at game {i+1}. Bankroll: {sim.get_current_bankroll():.2f}")
            break
        ...
        estimated_p_win = random.uniform(0.51, 0.55) # Example: fluctuating win prob, slightly positive edge
        estimated_net_odds = random.uniform(0.9, 1.2) # Example: fluctuating net odds (payout - 1)

        # Adjust bet size based on dynamic rules and Kelly criterion
        mm.adjust_bet_size()
        bet_amount = mm.get_recommended_bet_size(estimated_p_win, estimated_net_odds, kelly_fraction=0.3)
        bet_amount = min(bet_amount, sim.get_current_bankroll()) # Ensure bet doesn't exceed current bankroll
        
        if bet_amount <= 0:
            print(f"No funds or bet size too small to continue at game {i+1}. Bankroll: {sim.get_current_bankroll():.2f}")
            break
        ...
        if (i + 1) % 20 == 0:
            print(f"Game {i+1}: Bankroll={sim.get_current_bankroll():.2f}, Bet Size={bet_amount:.2f}, Max Drawdown={sim.get_max_drawdown():.2%}")

    print(f"\nFinal Bankroll: {sim.get_current_bankroll():.2f}")
    print(f"Max Drawdown: {sim.get_max_drawdown():.2%}")
    print(f"Probability of Ruin (conceptual): This would be derived from many such simulations.")
    print("Bankroll management simulation complete. Your financial fortress is now equipped with automated defenses, much like a seasoned strategist managing their war chest!")
